...
- `benchmarks/m2_path_ablation.py`, `m2_entity_query_ablation.py` and `m2_sim_reset_ablation.py` compare M2 host mapping/query paths and sparse reset transfer volume against Git baselines. They reuse test-owned fixtures, assert output/call equality and do not measure overall native throughput; see `docs/en/m2-ablation.md`.
- `benchmarks/issue141_fk_path_ablation.py` compares the legacy IsaacGym metadata/FK scan, reset staging and refresh publication against the #141 fix commit using NumPy-backed worker doubles.
