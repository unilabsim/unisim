...
Portable `entity_assets` extend this audited MJCF profile to no-variant or same-layout fixed-variant, no-mirror, physical fixed/floating entities and zero-DoF native rigid actors for fixed static entities. Each entity's public qpos/qvel, body, actuator, and contact identities map to frozen native actor layouts. Immutable fixed-variant assignments select independently materialized native realizations; native mass, inertia, and collision geometry may differ while public topology and addressing stay frozen. Entity-owned geom-pair contact sensors and selected reset-impacted control restoration are supported; variant changes to public topology, joint limits, actuator contracts, keyframes, or sensors, and mirrors, physical kinematic roots, and portable world-body plane contact sensors fail closed.
