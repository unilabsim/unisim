...
Portable `entity_assets` extend this audited MJCF profile to no-variant or same-layout fixed-variant physical fixed/floating entities, zero-DoF native rigid actors for fixed static entities, and bounded one-body collision-disabled mirrors of rigid sources. Each entity's public qpos/qvel, body, actuator, and contact identities map to frozen native actor layouts. Immutable fixed-variant assignments select independently materialized native realizations; native mass, inertia, and collision geometry may differ while public topology and addressing stay frozen. A mirror remains a public kinematic entity with zero joints, DoFs, and actuators; its hidden native free-root carrier supplies native state writes and assignment-selected source identity but has no collision, gravity, actuator, public state, or physical-wrench ownership. Mirror world-pose writes are row-local, full reset restores the declared mirror pose, and mirror contact sensors fail closed. Entity-owned geom-pair contact sensors and selected reset-impacted control restoration are supported; variant changes to public topology, joint limits, actuator contracts, keyframes, or sensors, physical kinematic roots, and portable world-body plane contact sensors fail closed.
