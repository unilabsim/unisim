...
Genesis' portable entity profile is a bounded MJCF subset: independent fixed/floating/passive/static entities bind by audited public names, inertia and addresses, and only single-link rigid heterogeneous assignments equal to Genesis' native balanced mapping are accepted. Public geometry is limited to cold-audited names, IDs and body ownership for named sphere/box visual instances; selected state/reset rows preserve unrelated entities and environments, while mirrors, kinematic entities, cross-entity sensors, reset randomization, geometry properties, body-force mapping, contact-mask aggregation and control restoration fail closed. Native CPU evidence uses Genesis 1.3.3, Torch 2.14.0+cpu and Quadrants 1.3.0; no GPU capability is claimed. See [entity scene execution](entity-scenes.md) for the exact variant and validation boundary.
