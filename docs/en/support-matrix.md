...
Mapped IsaacSim scenes expose frozen public geometry names, owning body IDs, normalized native collider masks, and per-environment immutable PhysX friction materials. Geometry dimensions and other contact-parameter reads remain unsupported; legacy model-file scenes fail closed. See [entity scene execution](entity-scenes.md) for the source-intent/native-readback boundary.
