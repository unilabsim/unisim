...
IsaacSim's raw-USD cache is a cold conversion optimization only. It does not cache native scenes, views, parameters, role-derived USD, or effective reports; hits still materialize and read back from the copied raw artifact. See [entity scene execution](entity-scenes.md) for the cache root, environment overrides, identity inputs, validation, and atomic publication behavior.
