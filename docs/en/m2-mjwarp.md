...
The pinned `mujoco_warp.forward(model, data)` has no selected-world argument. This implementation therefore uploads the prepared values in place and forwards the **existing main Data**, recomputing derived workspaces across the batch. It does not use `reset_data()` for selected entities, and does not use scratch-local world IDs that would select the wrong per-world variant fields. This is a correctness path, not a selective-forward performance claim.
