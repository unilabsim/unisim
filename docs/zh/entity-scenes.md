...
场景级跨 entity geom-pair contact fragment 仅接受 `data="found" num="1"` 声明。Genesis 会按名称、归属 link 与 active-environment rows 把每个公开 geom 绑定到精确原生 collision instance，按 fixed-variant assignment 路由这些 ID，并把 Genesis 公共批量 `get_contacts()` 的 geom ID 与 valid mask 作为无序 pair 匹配。该标志描述最近一次已完成物理 step；选中 state/reset 行会被清零，直到其下一次完成的 step。force/netforce 形式、同 entity pair、缺失或模糊的 collision 身份以及其他所有 contact 形式均快速失败。
