...
场景级 site 位姿 fragment 仅接受指向最终限定公开 site 名称、world-referenced 的 `framepos` 与 `framequat` 声明。其完整 site 身份必须在每个组合 fixed variant 中一致；读取使用与 entity 内 site 位姿 sensor 相同的公开原生 link/source frame 和已审计 site 局部位姿计算。场景级 body 目标、引用形式与其他所有 site fragment 形式均快速失败。
...
Entity-owned source site 运动 sensor 额外仅接受同一 entity 内具名 site 上未引用参考系的 `velocimeter` 与 `gyro` 声明。Motrix 必须把它们物化为原生 `FrameLinVel(local)` 与 `FrameAngVel(local)` sensor 且维度精确；fixed variants 也必须保持完整身份一致。其值是原生 site 坐标系运动向量：原生验收会把公开世界系 site Jacobian 结果旋转到已审计 site 坐标系并与读取值比较。accelerometer、body 运动目标、引用参考系 sensor 与场景级运动 fragment 均快速失败。
...
| Motrix | 支持无 variant 与不可变 same-layout fixed-variant、固定/浮动 root 的 MJCF 物理实体、被动标量关节、固定 rigid 静态实体、所选默认 keyframe 的构造/完整 reset 状态、世界系 body force/torque 扰动、生成的 body 系跟踪位姿 sensor、authored 世界系 body 位姿 sensor、entity 内 world-referenced site 位姿 sensor 与 entity 内 site 坐标系运动 sensor。 | 每个被使用的 variant 创建一个原生 Motrix model/data context；审计后的公开 state/control/sensor 行被 scatter/gather，局部 reset 保留无关状态，不支持的语义快速失败。 |
...
| Genesis | 有边界 CPU profile 的 portable MJCF 固定/浮动/被动/静态实体、单 link rigid 异构 variants、entity 内未引用参考系的 site 位姿/运动/identity 姿态 accelerometer sensor、场景级 world-referenced 限定 site 位姿 fragment，以及跨 entity geom-pair found/netforce contact。 | 独立原生实体按审计后的公开名称与地址绑定；局部 state/reset 行保留无关状态，审计后的原生 sphere/box 尺寸与完全一致的 contact mask 按公开顺序聚合，任何非 balanced assignment 或不支持的 sensor/DR 映射快速失败。 |
...
Genesis portable-entity 验收是真实原生 CPU 测试 `tests/adapters/genesis/test_portable_entities.py`。其 N5/K2 `[0,0,0,1,1]` assignment 覆盖受控固定根 robot、浮动被动 articulation、异构 rigid object 与固定静态 table；测试校验公开布局维度与 actuator 宽度、通过实际原生 link 名称/ID 绑定、原生 variant 质量 `[0.5,0.5,0.5,1.5,1.5]`、公开 geometry 名称/ID/body 归属、实际原生 geometry 尺寸、按公开顺序聚合的实际原生 contact mask、原生 DOF damping/friction-loss/armature 读回、active 原生 visual instance 及其源 AABB、局部 state 隔离、局部 reset 隔离、源 variant 惯量记录、joint 控制的物理响应、异构 free-body 旋转响应、选中行原生 COM shift 与重力驱动的相反 joint 响应、选中行 DOF 属性修改/原生读回与 damping 依赖响应，以及 fixed-variant 身份稳定的 site 位姿/运动 sensor 与局部 reset 行隔离。场景级限定 site fragment 会读取 passive 与异构 object 行，在局部 state 变化时保留无关行，并拒绝 variant site 身份漂移。geom-pair fragment gate 会让五行 object 均与 table 重叠，校验两个 object variant collision ID 与两个作者声明方向的力，检查重力下两种质量的原生支持力，在下一步前清空选中 reset 行，并保留其他行的已完成 step 标志与力。测试还会在一步后对比公开 accelerometer cache 与干净原生 IMU 读回，并拒绝引用参考系、旋转 accelerometer、同 entity contact 与其他不支持的 fragment/variant 身份漂移 sensor 声明，同时通过公开 size getter 拒绝故意非一致的 object variant 半径。SDK-free owner 测试覆盖缺失/variant 非一致 mask、同 variant 非一致 active 行 DOF 与有边界 sensor inventory 的拒绝。记录的运行时为 Genesis 1.3.3、Torch 2.14.0+cpu 与 Quadrants 1.3.0；这是 CPU 证据，不构成 GPU 声明。Genesis 测试被跳过不构成原生证据。

Motrix portable-entity 验收是真实原生测试 `tests/adapters/motrix/test_portable_entities.py`。它覆盖重复本地名称、两个浮动 root、被动关节物理响应、公开/原生 body 与 geom 映射、subtree ID、原生 mass/COM 与 geometry 尺寸读回、保留 control 的局部 reset 隔离、所选默认 keyframe qpos/qvel/control 的构造与完整 reset 恢复、带 activation 源的拒绝、被动 joint reset、带 direct/interval 一致性与局部 reset 取消范围的世界系 body torque、生成的 frame-position/quaternion sensor 读取与局部 reset 行隔离、与生成 sensor 混用的 authored 世界系 body 位姿 sensor、entity 内与场景级 fragment world-referenced site 位姿读取及局部 reset 隔离与 assignment gather、旋转 site 的 velocimeter/gyro 与公开世界系 site Jacobian 对比及局部 reset 隔离、五个不同 joint 角度下按 assignment 路由的具名 site 世界系 Jacobian、N5/K2 `[1,1,0,1,0]` same-layout assignment、不同有效标量 joint 惯量响应、选中行原生 armature/friction-loss 修改/读回与 armature 依赖响应，以及按 assignment 路由的生成/source/site 运动 sensor gather，还有模糊 base、包含 site accelerometer 在内的不支持 source sensor 与 fragment 拒绝、不支持 layout 拒绝和组合源清理。记录的本地证据使用 Python 3.13.14、MotrixSim Core 0.8.2、MuJoCo 3.11.0 与 NumPy 2.5.2；Motrix 测试被跳过不构成原生证据。
