...
| Motrix | 支持无 variant 与不可变 same-layout fixed-variant、固定/浮动 root 的 MJCF 物理实体、被动标量关节、固定 rigid 静态实体、生成的 body 系跟踪位姿 sensor、authored 世界系 body 位姿 sensor 和 entity 内 world-referenced site 位姿 sensor。 | 每个被使用的 variant 创建一个原生 Motrix model/data context；审计后的公开 state/control/sensor 行被 scatter/gather，局部 reset 保留无关状态，不支持的语义快速失败。 |
