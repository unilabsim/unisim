...
局部 `reset_entities()` 行通过 `SceneData[DisjointIndices]` 一致提交，不调用整场景 reset，并保留未提及通道、无关实体、无关环境与当前 control。`restore_default_controls=True` 时，只把被选中 entity root/joint patch 影响的 control 恢复为该 variant 冷路径捕获的原生默认值；无关 control 列与行保持不变。完整 `reset()` 会提交每个选中环境的完整 variant 默认 qpos/qvel 与默认 control，保留无关环境，并清除 reset 行上的全部 pending body force/torque。部分原生状态提交会使 backend 进入 faulted。其他 fixed-variant layout、kinematic mirrors、其他 source sensor 形式、其他 site sensor 形式、不支持的 contact 形式、生成 terrain、reset randomization、fixed-variant 原生播放/渲染与非一致公开 geometry 尺寸读取均快速失败。固定 link 的质量可能读为零，因为公开 mass/COM 是有效原生读回，不是源 inertial 元素的回显。
