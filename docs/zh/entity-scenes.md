...
IsaacSim 可缓存的内容仅限 expanded MJCF 到 raw USD 的转换输出。默认根目录为 `~/.cache/unisim/isaacsim/raw-usd`；可将 `UNISIM_ISAACSIM_RAW_USD_CACHE` 设为绝对路径，或使用只读兼容回退 `UNILAB_ISAACSIM_RAW_USD_CACHE`。空值或 `0`、`false`、`no`、`off`、`none`、`disabled`（不区分大小写）会禁用缓存。Raw identity 在 portable scene content identity 之上追加 expanded source digest、importer 参数、IsaacSim/IsaacLab/MJCF importer/Python 版本、实体和 variant。命中会校验完整 manifest、文件清单、大小和 SHA-256；转换输出先写入 staging 再原子发布，格式错误、中断或损坏条目按 miss 处理并替换。命中仍执行原生 materialization、prim/view 构建、参数读回、capability 检查和 effective-report 校验。worker 在 role bake 前复制不可变 raw artifact，因此 role 编辑不会修改缓存 raw USD。Role 派生缓存仍是独立 identity stage，不由本缓存提供。
...
Mapped IsaacSim 还支持作用于原生 body COM 的世界系 `body_force` 与 `body_torque` 区间 wrench。多次提交在下一次 step 前累加，作用于请求的每个子步，并在随后消费。Entity reset 清理选中实体 body，完整 reset 清理所有暂存 body 行；非 `None` 的逐子步宿主 callback 仍不支持。
