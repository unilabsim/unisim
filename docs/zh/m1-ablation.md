...
| 隔离操作 | 优化前 / A | 优化后 / B | 含义 |
| --- | --- | --- | --- |
| 配置比较，独立且相等的 `(32, 512, 3)` 值 | 31.935 ms | 16.464 ms | 序列化报告相同，保留独立快照 |
| 配置比较，共享输入对象 | 31.794 ms；Python 峰值 2,376,912 字节 | 8.124 ms；Python 峰值 1,191,512 字节 | 脱离输入的不可变快照，仅相同输入对象共享存储 |
| 能力序列化 | 0.078 ms | 0.031 ms | 序列化声明值相同 |
| CUDA `(4096, 128, 3)` 回读，仅消费一行 | 0.375 ms，复制所有行 | 0.016 ms，复制选中行 | 隔离传输机制，不是完整初始化耗时 |
| MuJoCo 构造 + 物化 | 关闭报告：7.857 ms | 开启报告：9.775 ms；严格路径：10.371 ms | 报告存在可测的冷路径成本 |
| MuJoCo 十个子步 | 关闭报告：0.293 ms | 开启报告：0.270 ms；严格路径：0.263 ms | 状态一致，计时差异不构成吞吐量结论 |
