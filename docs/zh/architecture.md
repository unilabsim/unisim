...
物理状态回放渲染是正式的后端契约。声明 `get_play_capabilities().supports_physics_state_playback` 的后端通过 `get_physics_state()` 提供快照，其行布局为 `[time, qpos, qvel]`；当模型含 mocap body 时追加 `[mocap_pos(nmocap*3), mocap_quat(nmocap*4)]` 尾段。布局由契约方法 `get_physics_state_layout()` 以 `PhysicsStateLayout`（`nq`、`nv`、`nmocap`）描述；渲染前端通过 `PhysicsStateLayout.split_state()` 解码快照，而不是硬编码列切片。录制的 mocap body 位姿经契约方法 `get_playback_mocap_state(env_index)` 暴露，并以 `supports_mocap_playback` 声明，因此交互与离线渲染器都不接触后端私有状态。`assert_backend_conformance` 会对声明该能力的后端校验契约：快照形状与布局一致性、播放模型可加载性（`MjModel` 或可编译的 MJCF/MJB 文件）、后端覆写 `set_physics_state` 时的 set/get round-trip，以及 mocap 尾段与所声明能力的一致性。MuJoCo、MJWarp、Newton、Drake、SuperDex、Motrix、Genesis、IsaacGym 与 IsaacSim 适配器均实现了该布局契约。Motrix 的原生场景数据不含仿真时钟，因此快照时间列由后端累积（每个公开 step 路径推进 `nsteps * sim_dt`，完整状态重置时清零）；其回放模型使用 whole-MJCF 构造源或组合后的 portable MJCF（拥有多个物化源的 fixed-variant 场景要求显式 `env_index`，并返回该行分配到的 variant 文件）；由于 kinematic mocap 镜像尚未纳入其快照尾段，本阶段其布局报告 `nmocap=0`。Genesis 的快照取自宿主 qpos/qvel 缓存（Genesis 的 free-base 与 ball joint 四元数原生即为 wxyz），时间列采用相同的后端累积语义；whole-MJCF 场景在物化时校验原生广义状态顺序与源 MJCF 关节顺序一致（fail-closed，复用共享的 MJCF 关节布局清单）；回放模型使用经 fragment 展开的构造源或组合后的 portable MJCF（fixed-variant 场景遵循同样的显式 `env_index` 规则）；在 kinematic 镜像 mocap 回放作为后续工作落地之前，其布局同样报告 `nmocap=0`。IsaacGym 在其 worker 侧采集批量状态块，并通过 `GET_PHYSICS_STATE` 请求/响应帧返回（不新增带外通道）；基于同样的缺失时钟原因，快照时间列由宿主机累积；raw-MJCF 场景在构造时对照源关节布局审计（固定基座或根顺序不符的场景声明回放能力不可用）；kinematic entity 根作为 mocap 尾段追加，并声明 `get_playback_mocap_state`。IsaacSim 由 host 侧从 worker 维护的共享内存槽装配快照，并因 worker 协议不携带仿真时钟而在 host 侧累积快照时间；原生 Kit viewer 与相机路径与回放契约并存。Mapped IsaacSim 场景直接复制公开顺序的 `qpos`/`qvel` 行（worker 在 INIT 时把 PhysX dof 映射到公开 MJCF 顺序），构造时对照编译得到的场景布局快速失败地校验组合回放 MJCF，fixed-variant 场景经显式 `env_index` 返回分配到的 variant 文件。Legacy whole-model 场景把 `root_state`/`dof_state` 线槽投影为相同的行 —— 将发布的世界系根角速度旋转到 body 系，使快照经 legacy `set_state` 线无损还原 —— 并通过 whole-MJCF 构造源回放；构造时校验源文件关节顺序与线协议的合成 7/6 根布局一致，拓扑无法被线协议回放（固定基座、ball 关节、自由根乱序）的场景保持该能力快速失败，但训练不受影响。IsaacSim 两个 profile 本阶段均报告 `nmocap=0`。
