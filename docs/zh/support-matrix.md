...
IsaacSim 的 raw USD 缓存只是冷路径转换优化。它不缓存原生场景、view、参数、role 派生 USD 或 effective report；命中仍基于复制的 raw artifact 物化并读回。缓存根目录、环境覆盖、身份输入、校验与原子发布行为见[实体场景执行](entity-scenes.md)。
