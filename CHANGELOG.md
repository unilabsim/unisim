...
- Consumed the mjbatch-uni 0.2.1 split-substep sensor copyout (unilabsim/mjbatch_uni#28) in the MuJoCo pre-step control path: with a capable executor, the tracked world-frame sensor views are memcpy-refreshed at every substep boundary (about 14x faster than the host-kinematics recompute at 4096 envs with bit-identical trajectories), while older executors keep the lazy host-side fallback. `uv.lock` now pins `mjbatch-uni==0.2.1`.
